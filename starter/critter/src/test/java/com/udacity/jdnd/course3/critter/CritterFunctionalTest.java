...
public class CritterFunctionalTest {
    ...
    private static void compareSchedules(ScheduleDTO sched1, ScheduleDTO sched2) {
        List<Long> sched2PetIds = sched2.getPetIds();
        Collections.sort(sched2PetIds);
        List<Long> sched2employeeIds = sched2.getEmployeeIds();
        Collections.sort(sched2employeeIds);

        List<Long> sched1PetIds = sched1.getPetIds();
        Collections.sort(sched1PetIds);
        List<Long> sched1employeeIds = sched1.getEmployeeIds();
        Collections.sort(sched1employeeIds);

        Assertions.assertEquals(sched1PetIds,sched2PetIds);
        Assertions.assertEquals(sched1.getActivities(), sched2.getActivities());
        Assertions.assertEquals(sched1employeeIds, sched2employeeIds);
